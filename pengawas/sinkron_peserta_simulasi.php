    ...
    if($json)
    {
		...
		if ($cek->num_rows > 0) 
		{
		   $stmt = $db->prepare("UPDATE siswa SET nama_siswa = ?, username = ?, password = ?, kelas = ?, rombel = ?, nis =? WHERE id_siswa = ? ");
			$stmt->bind_param("sssssss", $nama, $nis, $nis, $kelas, $rombel, $nis,$versi );
			$stmt->execute();
		} else 
		{
			$stmt = $db->prepare("INSERT INTO siswa (id_siswa, nama_siswa, username, password, kelas, rombel, nis) VALUES (?,?,?,?,?,?,?)");
			$stmt->bind_param("sssssss", $nis, $nama, $nis, $nis, $kelas, $rombel,$versi);
			$stmt->execute();
		}
	    }
        }
...
<?php
				$id++;
				$lanjut = 'sinkron_peserta_simulasi.php?id='.$id;
			?>
