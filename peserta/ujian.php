...
">
⚠ Anda belum menjawab soal selama 5 menit
</div>
...
let warningAktif = false;

function tampilkanWarning(menit)
{
    if(warningAktif) return;

    warningAktif = true;

    let div = document.getElementById('idleWarning');

    div.innerHTML =
        '⚠ Anda belum menjawab soal selama '
        + menit +
        ' menit';
...
async function cekIdle()
{
    try
    {
        let r = await fetch('cek_idle.php?t=' + Date.now());

        let data = await r.json();

        if(data.idle)
        {
            tampilkanWarning(data.menit);
        }
        else
        {
            sembunyikanWarning();
...
let intervalGetar = null;

function mulaiGetar()
{
    if(intervalGetar) return; // jangan dobel

    intervalGetar = setInterval(function(){
