...
$q = $db->query("SELECT
    s.id_siswa, s.username, s.kelas,
    s.nama_siswa,
    COUNT(j.id_soal) AS dijawab

FROM siswa s

INNER JOIN ujian_aktif ua
    ON ua.kelas = s.kelas
...
LEFT JOIN jawaban j
    ...
    AND j.id_ujian = ua.id_ujian
    AND j.jawaban IS NOT NULL
    AND TRIM(j.jawaban) <> ''

WHERE ua.id_ujian = '$id_ujian'
...
<tr>
    ...
    <td class="text-start"><a href="kirim_per_siswa.php?id_siswa=<?= $r['id_siswa'];?>&id_ujian=<?= $id_ujian;?>"><?= htmlspecialchars($r['nama_siswa']) ?></a></td>
    <td><?= $r['kelas'] ?></td>
    <td>
